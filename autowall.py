...
# Atalho padrão da empresa (um clique libera essa faixa)
COMPANY_PRESET_NAME = "PORTAS PLAYLIST"
COMPANY_PRESET_PORTS = "3030-3040"
...
# =============================
# CAMINHO DE RECURSOS (dev e empacotado)
# =============================
...
ICON_PATH = resource_path("autowall.ico")
...
def on_create_click():
    create_firewall_rules(entry_rule_name.get().strip(), entry_ports.get().strip())


def on_company_preset_click():
    create_firewall_rules(COMPANY_PRESET_NAME, COMPANY_PRESET_PORTS)
...
if __name__ == "__main__":
    ...
    # Define o ícone da janela (barra de título e taskbar).
    # O customtkinter costuma sobrescrever o ícone logo após iniciar,
    # por isso reaplicamos com after().
    ...
    info_label = ctk.CTkLabel(
        ...
        text=(
            ...
            f"- Atalho: libera {COMPANY_PRESET_PORTS} com 1 clique"
        ),
        justify="left",
        anchor="w",
    ...
    btn_create.pack(fill="x", padx=24, pady=(0, 12))

    btn_preset = ctk.CTkButton(
        frame,
        text=f"⚡ Liberar {COMPANY_PRESET_NAME} ({COMPANY_PRESET_PORTS})",
        height=40,
        corner_radius=12,
        fg_color="#1d4ed8",
        hover_color="#1e40af",
        ...
        command=on_company_preset_click,
    )
    btn_preset.pack(fill="x", padx=24, pady=(0, 22))
